...
function PerAccountTab() {
  ...
  const balanceVnd = data?.balanceVnd ?? 0;
...
function PerKeyTab() {
  ...
  const [startDate, setStartDate] = useState(defaults.from);
  const [endDate, setEndDate] = useState(defaults.to);
  const { data: keys, isLoading } = useKeyUsage(startDate, endDate);
        ...
        {keyList.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {keyList.map((k) => (
                  <tr key={k.keyId} className="border-b border-border/50 last:border-0">
                    <td className="py-2.5">
                    ...
                    <td className="py-2.5 text-right font-medium text-foreground">
                      {k.costVnd.toLocaleString("vi-VN")}d
                    </td>
